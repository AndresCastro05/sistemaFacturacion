...
import com.bolsadeideas.springboot.app.util.paginator.PageRander;
...
public class ClienteController {
	...
	public ResponseEntity<Resource> verFoto(@PathVariable String filename) {
		...
		return ResponseEntity.ok().header(org.springframework.http.HttpHeaders.CONTENT_DISPOSITION,
				"attchment; filename=\"" + recurso.getFilename() + "\"").body(recurso);
	...
	public String listar(@RequestParam(name = "page", defaultValue = "0") int page, Model model) {
		...
		PageRander<Cliente> pageRender = new PageRander<>("/listar", clientes);
	...
	public String editar(@PathVariable(value = "id") Long id, Map<String, Object> model, RedirectAttributes flash) {
		...
		if (id > 0) {
			...
			if (cliente == null) {
				flash.addFlashAttribute("error", "El ID del cliente existe en la BBDD");
				return "redirect:/listar";
			}
		} else {
		...
		model.put("cliente", cliente);
		model.put("titulo", "Formulario de editar cliente");
		return "form";
	}
